window.Fn = window.Fn || {}; // Asegura que no se sobreescriba

window.Fn.validaRut = function (rutCompleto) {
    if (!/^[0-9]+[-|‐]{1}[0-9kK]{1}$/.test(rutCompleto)) return false;
    var tmp = rutCompleto.split('-');
    var digv = tmp[1];
    var rut = tmp[0];
    if (digv == 'K') digv = 'k';
    return window.Fn.dv(rut) == digv;
};

window.Fn.dv = function (T) {
    var M = 0,
        S = 1;
...
console.log("✅ validaRut.js cargado correctamente y window.Fn definido", window.Fn);